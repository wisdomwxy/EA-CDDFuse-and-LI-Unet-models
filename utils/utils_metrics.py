...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):  
    ...
    # 计算F1-Score
    F1_Score    = 2 * (Precision * PA_Recall) / (Precision + PA_Recall + 1e-6)
    ...
    if name_classes is not None:
        for ind_class in range(num_classes):
            print('===>' + name_classes[ind_class] + ':\tIou-' + str(round(IoUs[ind_class] * 100, 2)) \
                ...
                + '; F1-Score-' + str(round(F1_Score[ind_class] * 100, 2)))
    ...
    print('===> mIoU: ' + str(round(np.nanmean(IoUs) * 100, 2)) \
        ...
        + '; mF1-Score: ' + str(round(np.nanmean(F1_Score) * 100, 2)) \
        + '; Accuracy: ' + str(round(per_Accuracy(hist) * 100, 2)))
    
    return np.array(hist, np.int32), IoUs, PA_Recall, Precision, F1_Score
...
def show_results(miou_out_path, hist, IoUs, PA_Recall, Precision, F1_Score, name_classes, tick_font_size = 12):
    ...
    draw_plot_func(F1_Score, name_classes, "mF1-Score = {0:.2f}%".format(np.nanmean(F1_Score)*100), "F1-Score", \
        os.path.join(miou_out_path, "F1_Score.png"), tick_font_size = tick_font_size, plt_show = False)
    print("Save F1-Score out to " + os.path.join(miou_out_path, "F1_Score.png"))
